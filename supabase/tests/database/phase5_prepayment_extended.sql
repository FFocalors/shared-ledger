...
with x as (select * from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000012','f6200000-0000-0000-0000-000000000011',100,'2026-08-31 09:00+08',null))
insert into phase5_ids select 'normal_prepaid',transfer_id from x;
select pg_temp.assert_true(
 (select settlement_amount=0 and prepayment_amount=50 and currency='CNY' and financial_version=1 from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000003','f6200000-0000-0000-0000-000000000032','f6200000-0000-0000-0000-000000000031',50,'2026-08-31 09:01+08',null)),
...
with x as (select * from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000012','f6200000-0000-0000-0000-000000000011',100,'2026-08-31 12:00+08',null)) insert into phase5_ids select 'mixed_prepaid',transfer_id from x;
...
with x as (select * from pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000012','f6200000-0000-0000-0000-000000000011',20,'2026-08-31 13:00+08',null)) insert into phase5_ids select 'partial_return',transfer_id from x;
...
do $test$ begin begin perform pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000012','f6200000-0000-0000-0000-000000000011',21,now(),null); raise exception 'over-return succeeded'; exception when check_violation then null; end; end $test$;
...
select pg_temp.assert_true(
  ...
  and (select balance=100 from public.prepayment_accounts where activity_id='f6000000-0000-0000-0000-000000000002')
  and not exists(select 1 from public.bilateral_debts where activity_id='f6000000-0000-0000-0000-000000000002'),
  'fully refunded linked Expense releases its actual Usage without creating reverse debt'
);

-- A linked refund permanently locks its parent’s financial fields. Presentation
-- remains editable, and soft-deleting a child refund rebuilds the projection.
...
begin
  ...
  perform pg_temp.assert_true(
    (select original_amount=50 and title='original' and version=1 and not is_deleted and financial_locked
     from public.expenses where id=(select object_id from phase5_ids where label='refund_original'))
    and (select financial_version=v_version from public.activities where id='f6000000-0000-0000-0000-000000000002'),
    'rejected parent financial update preserves refund source facts and version'
  );
end;
$parent_financial_lock$;
select pg_temp.assert_true((select financial_locked from public.update_expense_presentation(
 (select object_id from phase5_ids where label='refund_original'),'original renamed','presentation update','money',
 (select version from public.expenses where id=(select object_id from phase5_ids where label='refund_original')))),
 'presentation-only parent update remains allowed after linked refund');
...
do $negative$ begin begin
  perform * from pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000002','f6200000-0000-0000-0000-000000000022','f6200000-0000-0000-0000-000000000021',1,now(),null);
...
do $archived$ begin begin perform * from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000012','f6200000-0000-0000-0000-000000000011',1,now(),null); raise exception 'archived write succeeded'; exception when sqlstate '55000' then null; end; end $archived$;
...
-- An activity member may create a prepayment for two participants without
-- claiming either participant; the v2 contract does not accept a proxy actor.
reset role; set local role service_role;
...
with x as (select * from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000013','f6200000-0000-0000-0000-000000000011',5,now(),null))
insert into phase5_ids select 'unclaimed_participant_transfer',transfer_id from x;
grant select on phase5_ids to service_role;
reset role; set local role service_role;
select pg_temp.assert_true((select t.recorded_by='f6300000-0000-0000-0000-000000000001' and t.on_behalf_of_participant_id is null from public.transfers t where t.id=(select object_id from phase5_ids where label='unclaimed_participant_transfer')), 'v2 prepayment records the activity member without a proxy actor');
set local role authenticated; select pg_temp.authenticate('f6300000-0000-0000-0000-000000000001');
do $proxy$ begin begin perform pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000001','f6200000-0000-0000-0000-000000000012','f6200000-0000-0000-0000-000000000011',1,now(),'f6200000-0000-0000-0000-000000000013'); raise exception 'v2 accepted a proxy actor'; exception when sqlstate '42501' then null; end; end $proxy$;
reset role;
...
with x as (select * from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',100,now(),null)) insert into phase5_ids select 'edge_funded',transfer_id from x;
with x as (select * from pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',40,now(),null)) insert into phase5_ids select 'edge_return',transfer_id from x;
...
with x as (select * from pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',60,now(),null)) insert into phase5_ids select 'edge_return_all',transfer_id from x;
...
do $edge_over$ begin begin perform pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',1,now(),null); raise exception 'over return succeeded'; exception when check_violation then null; end; end $edge_over$;
with x as (select * from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',100,now(),null)) insert into phase5_ids select 'edge_deposit_active',transfer_id from x;
with x as (select * from pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',40,now(),null)) insert into phase5_ids select 'edge_return_active',transfer_id from x;
...
select pg_temp.assert_true((select count(*)=1 from pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',1,now(),null)), 'claimed member can create using own participant');
do $edge_behalf$ begin begin perform pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',1,now(),'f6200000-0000-0000-0000-000000000041'); raise exception 'member forged behalf'; exception when sqlstate '42501' then null; end; end $edge_behalf$;
set local role authenticated; select pg_temp.authenticate('f6300000-0000-0000-0000-000000000004');
do $edge_outsider$ begin begin perform pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',1,now(),null); raise exception 'outsider succeeded'; exception when sqlstate '42501' then null; end; end $edge_outsider$;
...
do $edge_arch$ begin begin perform pg_temp.create_prepayment_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',1,now(),null); raise exception 'archived create succeeded'; exception when sqlstate '55000' then null; end; begin perform pg_temp.create_prepayment_return_fixture('f6000000-0000-0000-0000-000000000004','f6200000-0000-0000-0000-000000000042','f6200000-0000-0000-0000-000000000041',1,now(),null); raise exception 'archived return succeeded'; exception when sqlstate '55000' then null; end; begin perform public.void_prepayment_transfer((select object_id from phase5_ids where label='edge_return_active'),'archived void'); raise exception 'archived void succeeded'; exception when sqlstate '55000' then null; end; end $edge_arch$;
