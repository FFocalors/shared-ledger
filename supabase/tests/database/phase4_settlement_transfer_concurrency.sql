...
-- sessions can see it. It lives only in the throwaway database created by
-- Invoke-IsolatedDatabaseTests.ps1 and is removed with that database.
...
select extensions.dblink_exec(
  'phase4_setup',
  $sql$
    create function public.phase4_concurrency_attempt(p_request_id uuid, p_on_behalf uuid)
    returns text
    language plpgsql
    volatile
    security invoker
    ...
    $attempt$;
    revoke all on function public.phase4_concurrency_attempt(uuid,uuid) from public, anon, authenticated;
    grant execute on function public.phase4_concurrency_attempt(uuid,uuid) to authenticated
  $sql$
);
  ...
  $sql$
    ...
    begin
      ...
      perform * from public.create_expense_auto_rate(
        ...
        'Concurrent debt', 100, 'CNY', 'manual',
...
select extensions.dblink_send_query(
  'phase4_attempt_a',
  'select public.phase4_concurrency_attempt(''f5400000-0000-0000-0000-000000000001'',''f5200000-0000-0000-0000-000000000002'')'
);
select extensions.dblink_send_query(
  'phase4_attempt_b',
  'select public.phase4_concurrency_attempt(''f5400000-0000-0000-0000-000000000002'',null)'
);
...
select pg_temp.assert_true(
  ...
  and (select count(*) = 1 from phase4_concurrency_results where result = '40001'),
...
select pass('Phase 4 settlement concurrency assertions');
