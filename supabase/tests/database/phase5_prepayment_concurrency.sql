...
select extensions.dblink_connect('p5_setup',:'db_conninfo');
...
select extensions.dblink_exec('p5_setup',$sql$
 ...
 revoke all on function public.p5_return_attempt(uuid) from public, anon, authenticated; grant execute on function public.p5_return_attempt(uuid) to authenticated;
 set role authenticated; do $claims$ begin perform set_config('request.jwt.claims','{"sub":"f7500000-0000-0000-0000-000000000001","role":"authenticated"}',false); end $claims$;
 do $seed$ begin perform * from public.create_prepayment_v2('f7600000-0000-0000-0000-000000000001','f7800000-0000-0000-0000-000000000001','f7800000-0000-0000-0000-000000000002',100,'CNY',now(),null,0,'f7900000-0000-0000-0000-000000000001'); end $seed$;
 commit
$sql$); select extensions.dblink_disconnect('p5_setup');
select extensions.dblink_connect('p5_lock',:'db_conninfo');
select extensions.dblink_connect('p5_a',:'db_conninfo');
select extensions.dblink_connect('p5_b',:'db_conninfo');
...
select extensions.dblink_send_query('p5_a','select public.p5_return_attempt(''f7900000-0000-0000-0000-000000000002'')'); select extensions.dblink_send_query('p5_b','select public.p5_return_attempt(''f7900000-0000-0000-0000-000000000003'')'); select pg_sleep(.2);
...
select pg_temp.assert_true((select count(*)=1 from p5_results where result='ok') and (select count(*)=1 from p5_results where result='40001'),'one 60 return succeeds and the stale same-version attempt is rejected');
...
select pass('Phase 5 prepayment return concurrency assertions');
