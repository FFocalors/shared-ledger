...
-- see it. It remains isolated to the throwaway database runner.
select extensions.dblink_connect('phase6_setup',:'db_conninfo');
...
select extensions.dblink_exec('phase6_setup',$sql$
  ...
  create function public.phase6_final_attempt(p_request_id uuid) returns text language plpgsql volatile security invoker set search_path='' as $attempt$
  begin
    perform * from public.execute_final_settlement_v2(
      'f8700000-0000-0000-0000-000000000001',
      'f8720000-0000-0000-0000-000000000001',
      'f8720000-0000-0000-0000-000000000003',
      100,'CNY','base_unified',2,p_request_id,pg_catalog.now(),null
    );
    return 'ok';
  exception when others then return sqlstate;
  end;
  $attempt$;
  revoke all on function public.phase6_final_attempt(uuid) from public,anon,authenticated;
  grant execute on function public.phase6_final_attempt(uuid) to authenticated;
  set local role authenticated;
  do $claims$ begin perform set_config('request.jwt.claims','{"sub":"f8700000-0000-0000-0000-000000000001","role":"authenticated"}',true); end $claims$;
  do $setup_expenses$
  begin
    perform * from public.create_expense_auto_rate('f8710000-0000-0000-0000-000000000001','root debt',100,'CNY','manual','[{"participant_id":"f8720000-0000-0000-0000-000000000002","amount":"100"}]','[{"participant_id":"f8720000-0000-0000-0000-000000000001","amount":"100"}]','{}',now(),null,null,'money');
    perform * from public.create_expense_auto_rate('f8710000-0000-0000-0000-000000000002','child debt',100,'CNY','manual','[{"participant_id":"f8720000-0000-0000-0000-000000000003","amount":"100"}]','[{"participant_id":"f8720000-0000-0000-0000-000000000002","amount":"100"}]','{}',now(),null,null,'money');
  end
  $setup_expenses$;
$sql$);
...
select extensions.dblink_connect('phase6_lock',:'db_conninfo');
select extensions.dblink_connect('phase6_attempt_a',:'db_conninfo');
select extensions.dblink_connect('phase6_attempt_b',:'db_conninfo');
...
select extensions.dblink_exec('phase6_attempt_b',$sql$do $claims$ begin perform set_config('request.jwt.claims','{"sub":"f8700000-0000-0000-0000-000000000001","role":"authenticated"}',false); end $claims$;$sql$);
...
select extensions.dblink_send_query('phase6_attempt_a','select public.phase6_final_attempt(''f8730000-0000-0000-0000-000000000001'')');
select extensions.dblink_send_query('phase6_attempt_b','select public.phase6_final_attempt(''f8730000-0000-0000-0000-000000000002'')');
...
select pg_temp.assert_true((select count(*)=1 from phase6_concurrency_results where result='ok') and (select count(*)=1 from phase6_concurrency_results where result='40001'),'exactly one concurrent final execution succeeds and the stale-version loser is rejected');
...
-- Committed fixtures are discarded with the isolated test database.
