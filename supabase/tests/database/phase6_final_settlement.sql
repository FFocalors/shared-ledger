...
select pg_temp.assert_true((select base_amount=41 and version=2 from pg_temp.update_expense_fixture((select object_id from phase6_ids where label='root_debt'),'f8610000-0000-0000-0000-000000000001','root debt increased',41,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000012","amount":"41"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000011","amount":"41"}]','{}','2026-08-31 09:00+08',null,null)),'root update preserves the two-hop shape');
do $stale$ begin begin
  perform * from pg_temp.execute_final_settlement_fixture('f8600000-0000-0000-0000-000000000001','f8620000-0000-0000-0000-000000000011','f8620000-0000-0000-0000-000000000001',40,now(),null);
...
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000001');
with x as (select * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000001','f8620000-0000-0000-0000-000000000011','f8620000-0000-0000-0000-000000000001',41,'2026-08-31 13:00+08',null)) insert into phase6_ids select 'final_ac',transfer_id from x;
select pg_temp.assert_true(
...
-- Once final allocation paths capture source Expenses, financial edits and
-- deletion are permanently rejected while the settlement remains history.
...
do $final_source_lock$
begin
  begin
    perform * from pg_temp.update_expense_fixture(
      (select object_id from phase6_ids where label='root_debt'),
      'f8610000-0000-0000-0000-000000000001','root changed after final',20,'CNY',1,'manual',
      '[{"participant_id":"f8620000-0000-0000-0000-000000000012","amount":"20"}]',
      '[{"participant_id":"f8620000-0000-0000-0000-000000000011","amount":"20"}]',
      '{}','2026-08-31 09:00+08',null,null
    );
    raise exception 'final-settled source Expense update unexpectedly succeeded';
  exception when sqlstate '23514' then null;
  end;

  begin
    perform * from public.delete_expense(
      (select object_id from phase6_ids where label='root_debt')
    );
    raise exception 'final-settled source Expense deletion unexpectedly succeeded';
  exception when sqlstate '23514' then null;
  end;
end;
$final_source_lock$;
select pg_temp.assert_true(
  (select original_amount=41 and base_amount=41 and version=2 and not is_deleted
     from public.expenses where id=(select object_id from phase6_ids where label='root_debt'))
  and (select financial_version=(select financial_version from phase6_history_version)
     from public.activities where id='f8600000-0000-0000-0000-000000000001')
  and (select count(*)=2 and bool_and(source_expense_id in (
       select object_id from phase6_ids where label in ('root_debt','child_debt')))
       from public.final_settlement_paths where transfer_id=(select object_id from phase6_ids where label='final_ac')),
  'final source finance lock preserves Expense, path history, and Activity version after rejected edits'
);
...
do $normal_execute$ begin begin
  perform * from pg_temp.create_final_settlement_fixture(
...
with x as (select * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000006','f8620000-0000-0000-0000-000000000061','f8620000-0000-0000-0000-000000000063',1,now(),'f8620000-0000-0000-0000-000000000063')) insert into phase6_ids select 'capacity_final',transfer_id from x;
select pg_temp.assert_true(
...
with x as (select * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000008','f8620000-0000-0000-0000-000000000083','f8620000-0000-0000-0000-000000000082',100,'2026-08-31 17:00+08','f8620000-0000-0000-0000-000000000082')) insert into phase6_ids select 'weak_final',transfer_id from x;
...
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000001');
with x as (select * from pg_temp.create_prepayment_fixture('f8600000-0000-0000-0000-000000000007','f8620000-0000-0000-0000-000000000071','f8620000-0000-0000-0000-000000000072',10,'2026-08-31 18:00+08',null)) insert into phase6_ids select 'reverse_return_deposit',transfer_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000012','return owner to middle',10,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000073","amount":"10"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000071","amount":"10"}]','{}',now(),null,null)) insert into phase6_ids select 'reverse_om',expense_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000012','return middle to custodian',10,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000072","amount":"10"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000073","amount":"10"}]','{}',now(),null,null)) insert into phase6_ids select 'reverse_mc',expense_id from x;
...
with x as (select * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000007','f8620000-0000-0000-0000-000000000071','f8620000-0000-0000-0000-000000000072',10,'2026-08-31 19:00+08',null)) insert into phase6_ids select 'reverse_ordinary_final',transfer_id from x;
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000002');
with x as (select * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000007','f8620000-0000-0000-0000-000000000072','f8620000-0000-0000-0000-000000000071',10,'2026-08-31 20:00+08',null)) insert into phase6_ids select 'reverse_return_final',transfer_id from x;
...
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000001');
with x as (select * from pg_temp.create_prepayment_fixture('f8600000-0000-0000-0000-000000000007','f8620000-0000-0000-0000-000000000071','f8620000-0000-0000-0000-000000000073',100,'2026-08-31 21:00+08',null)) insert into phase6_ids select 'multihop_account',transfer_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000011','multihop direct account debt',50,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000073","amount":"50"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000071","amount":"50"}]','{}',now(),null,null)) insert into phase6_ids select 'multihop_direct_debt',expense_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000011','multihop A to middle',100,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000072","amount":"100"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000071","amount":"100"}]','{}',now(),null,null)) insert into phase6_ids select 'multihop_ab',expense_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000012','multihop middle to endpoint',100,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000073","amount":"100"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000072","amount":"100"}]','{}',now(),null,null)) insert into phase6_ids select 'multihop_bc',expense_id from x;
-- An effective final path is already included in the ordinary balance
-- reconstruction, so it cannot be counted again as an additional Usage.
select pg_temp.assert_true((select count(*) filter (where from_participant_id='f8620000-0000-0000-0000-000000000071' and to_participant_id='f8620000-0000-0000-0000-000000000073' and ordinary_amount=100 and prepayment_return_amount=0 and amount=100)=1 from public.preview_final_settlement('f8600000-0000-0000-0000-000000000007')),'multi-hop preview preserves the directed endpoint amount after final path allocations');
select pg_temp.assert_true((select sum(balance)=50 from public.prepayment_accounts where activity_id='f8600000-0000-0000-0000-000000000007' and owner_participant_id='f8620000-0000-0000-0000-000000000071' and custodian_participant_id='f8620000-0000-0000-0000-000000000073'),'multi-hop endpoint shares an account and preserves the correctly rebuilt residual before payment');
with x as (select * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000007','f8620000-0000-0000-0000-000000000071','f8620000-0000-0000-0000-000000000073',100,'2026-08-31 22:00+08',null)) insert into phase6_ids select 'multihop_final',transfer_id from x;
select pg_temp.assert_true((select count(*)=2 and sum(amount)=200 and bool_and(amount=100) from public.final_settlement_paths where transfer_id=(select object_id from phase6_ids where label='multihop_final')) and (select sum(balance)=50 from public.prepayment_accounts where activity_id='f8600000-0000-0000-0000-000000000007' and owner_participant_id='f8620000-0000-0000-0000-000000000071' and custodian_participant_id='f8620000-0000-0000-0000-000000000073') and (select sum(amount)=50 from public.prepayment_usages where activity_id='f8600000-0000-0000-0000-000000000007'),'multi-hop final endpoint does not double-count effective paths as Usage or direct account settlement');
...
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000002');
with x as (select * from pg_temp.create_prepayment_fixture('f8600000-0000-0000-0000-000000000003','f8620000-0000-0000-0000-000000000031','f8620000-0000-0000-0000-000000000032',10,'2026-08-31 14:00+08',null)) insert into phase6_ids select 'merge_prepay',transfer_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000006','custodian owes owner',20,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000031","amount":"20"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000032","amount":"20"}]','{}','2026-08-31 15:00+08',null,null)) insert into phase6_ids select 'merge_debt',expense_id from x;
...
with x as (select * from pg_temp.execute_final_settlement_fixture('f8600000-0000-0000-0000-000000000003','f8620000-0000-0000-0000-000000000032','f8620000-0000-0000-0000-000000000031',30,'2026-08-31 16:00+08',null)) insert into phase6_ids select 'merge_final',transfer_id from x;
...
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000001');
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000007','archive debt',5,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000042","amount":"5"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000041","amount":"5"}]','{}',now(),null,null)) insert into phase6_ids select 'archive_expense',expense_id from x;
with x as (select * from pg_temp.create_expense_fixture('f8610000-0000-0000-0000-000000000007','archive settled portion',5,'CNY',1,'manual','[{"participant_id":"f8620000-0000-0000-0000-000000000042","amount":"5"}]','[{"participant_id":"f8620000-0000-0000-0000-000000000041","amount":"5"}]','{}',now(),null,null)) insert into phase6_ids select 'archive_settled_expense',expense_id from x;
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000002');
with x as (select * from pg_temp.create_settlement_transfer_fixture('f8600000-0000-0000-0000-000000000004','f8620000-0000-0000-0000-000000000041','f8620000-0000-0000-0000-000000000042',5,now(),null)) insert into phase6_ids select 'archive_settlement',transfer_id from x;
select pg_temp.authenticate('f8600000-0000-0000-0000-000000000001');
...
do $archived$ begin
  ...
  begin perform * from pg_temp.create_settlement_transfer_fixture('f8600000-0000-0000-0000-000000000004','f8620000-0000-0000-0000-000000000042','f8620000-0000-0000-0000-000000000041',1,now(),null); raise exception 'archived settlement succeeded'; exception when object_not_in_prerequisite_state then null; end;
  begin perform * from pg_temp.create_prepayment_fixture('f8600000-0000-0000-0000-000000000004','f8620000-0000-0000-0000-000000000041','f8620000-0000-0000-0000-000000000042',1,now(),null); raise exception 'archived prepayment succeeded'; exception when object_not_in_prerequisite_state then null; end;
  begin perform * from pg_temp.create_prepayment_return_fixture('f8600000-0000-0000-0000-000000000004','f8620000-0000-0000-0000-000000000041','f8620000-0000-0000-0000-000000000042',1,now(),null); raise exception 'archived return succeeded'; exception when object_not_in_prerequisite_state then null; end;
  begin perform * from pg_temp.create_final_settlement_fixture('f8600000-0000-0000-0000-000000000004','f8620000-0000-0000-0000-000000000042','f8620000-0000-0000-0000-000000000041',1,now(),null); raise exception 'archived final succeeded'; exception when object_not_in_prerequisite_state then null; end;
  ...
  begin perform * from public.create_sub_activity('f8600000-0000-0000-0000-000000000004','blocked sub'); raise exception 'archived sub activity succeeded'; exception when insufficient_privilege then null; end;
end $archived$;
