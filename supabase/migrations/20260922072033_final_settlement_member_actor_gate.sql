...
-- Final settlement is a fact between activity participants.  Any authenticated
-- activity member may record the current plan; participant claims are not an
-- authorization prerequisite here.  Ordinary transfers and prepayments keep
-- using private.authorize_phase5_actor in their existing implementations.
...
begin
  ...
  if p_on_behalf_of_participant_id is not null
     and p_on_behalf_of_participant_id not in (p_from_participant_id, p_to_participant_id) then
